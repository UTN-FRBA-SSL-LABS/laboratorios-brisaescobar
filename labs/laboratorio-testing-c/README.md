...
> R:Inicializa un carrito con cantidad = 0 
...
> R: linea 24 recorre los "productos" del carrito y calcula el total segun su precio

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: total += c->items[i].precio; pero falta multiplicar por cantidad
...
> R: el operador <= MAX_ITEMS deberia ser < MAX_ITEMS
...
> R: prueba una función de forma aislada, mientras que un test de integración verifica cómo funcionan varias partes del sistema juntas. El bug de carrito_total fue detectado primero por el test unitario, específicamente por el test con cantidad mayor a 1.
...
> R: En C los arrays no realizan comprobación automática de límites, por lo que escribir fuera de sus posiciones válidas puede sobrescribir otras zonas de memoria y provocar corrupción de datos, errores o incluso vulnerabilidades de seguridad.
...
> R:Los tests permiten comprobar el comportamiento esperado de forma objetiva y reproducible. Un bug puede no ser evidente al leer el código, pero un test puede demostrar que para una entrada determinada el resultado obtenido no coincide con el esperado
...
> R:Porque el bug en carrito_total hacía que se sumara solamente el precio unitario y no se multiplicara por la cantidad. Cuando la cantidad es 1, precio × 1 es igual al precio unitario, por lo que el resultado incorrecto coincide con el esperado. Con cantidad 2, debería calcular precio × 2, pero el bug suma solamente el precio, haciendo que el test falle.
